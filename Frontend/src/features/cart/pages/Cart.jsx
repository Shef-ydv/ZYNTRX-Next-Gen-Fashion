...
const Cart = () => {
    ...
    const calculateTotal = () => {
        ...
        return cart.items.reduce((total, item) => {
            ...
            const variantDetail = getVariantDetails(product, item.variant)

            const finalPrice =
                item.price?.amount ??
                variantDetail?.price?.amount ??
                product?.price?.amount ??
                0

            const qty = quantities[product?._id] ?? item.quantity ?? 1

            return total + Number(finalPrice) * Number(qty)
        }, 0)
    }
                                        ...
                                        >
                                            ...
                                            <div className="flex-1 flex flex-col justify-between">
                                                <div>
                                                    ...
                                                    {displayPrice?.amount && variantPrice?.amount && displayPrice.amount !== variantPrice.amount && (
                                                        <>
                                ...
                                <div className="flex flex-col gap-4 mb-6">
                                    ...
                                    <div className="flex justify-between items-baseline">
                                        ...
                                        <span
                                            ...
                                            style={{ color: totalPrice >= 15000 ? '#5a7a5a' : tokens.muted }}
                                        >
                                            {totalPrice >= 15000 ? 'Complimentary' : 'Complimentary over INR 15,000'}
                                        </span>
                                    </div>
